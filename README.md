...
- **可编辑文件生成 API**：`/v1/ppts`、`/v1/psds`——对话式驱动 ChatGPT 代码解释器产出可编辑 `.pptx` / 分层 `.psd`（含素材 zip），仅调付费级 Web 账号，按任务固定价扣费。
...
- **可编辑文件异步任务态**：`/v1/ppts`、`/v1/psds` 当前为同步 keep-alive；后续补 `queued → 轮询` 异步任务态与 `client_task_id` 任务级幂等（当前为计费层幂等）。
