...
# 将 .env 中的 GPT2IMAGE_IMAGE_TAG 设为发布页上的完整版本，例如 v0.7.0。
docker compose pull
docker compose up -d
```
...
`GPT2IMAGE_IMAGE_NAMESPACE` 控制镜像命名空间，默认是上游发布仓库
`ghcr.io/meowfree`；fork 自行通过 GitHub Actions 发布镜像时，可改为自己的
GHCR 命名空间，例如 `ghcr.io/shuai-s`。`GPT2IMAGE_IMAGE_TAG` 是必填的完整
exact 版本（`vX.Y.Z` 或 `vX.Y.Z-(alpha|beta|rc).N`），同时控制 Web、数据库迁移和两个
sidecar。Compose 不接受空值，也不再默认 `latest`，避免四个仓库在发布中途
形成混合版本。GitHub Release 附带的 compose 包已把该变量钉到本次版本。

`gpt2image-pro-release` 是版本描述符仓库，不是可运行镜像。它的 OCI 顶层注解
记录版本和四组件的 `repository@sha256:digest`；正式版只移动 descriptor 的
`latest`，预发布只移动 `prerelease`。需要检查 channel 当前指向的完整组件集时：

```bash
docker buildx imagetools inspect --raw \
  ghcr.io/meowfree/gpt2image-pro-release:latest | jq '.annotations'
```

部署仍只消费四组件的同一个 exact tag，不直接运行 descriptor，也不要把组件
标签改成 `latest`。
...
升级到新版本时，先把 `.env` 中的 `GPT2IMAGE_IMAGE_TAG` 一次性改成目标 exact
版本，再拉取并重建四个组件：
...
仓库发布地址为 `MeowFree/GPT2Image-Pro`。推送到 `main` 或打 `v*` 版本 tag
会触发 GitHub Actions 构建 run-scoped GHCR 镜像；只有合法版本 tag 会提升为
正式发布并创建 GitHub Release 草稿：
...
- 发布 `ghcr.io/meowfree/gpt2image-pro-release` OCI descriptor
- 创建 GitHub Release 草稿，并附带 compose 部署包

四组件只发布不可变的完整 `vX.Y.Z[-prerelease]` 与 `X.Y.Z[-prerelease]` exact
标签，不再移动各自的 major、minor 或 `latest`。全部 exact 标签与 descriptor
写入成功后，工作流才通过一次 registry 操作移动 descriptor channel。
