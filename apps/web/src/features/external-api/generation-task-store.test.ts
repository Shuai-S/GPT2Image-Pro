 ...
 * running 接管、fencing 终态和耗尽重试收敛语义；不测试数据库驱动本身。
 */
...
describe("generation task lease store", () => {
  ...
  it("续租、重排与终态都限定 image/video 和当前 fencing token", async () => {
    ...
    await expect(
      requeueGenerationTask("task-1", "lease-token-1", 250)
    ).resolves.toBe(true);
    await expect(
      finalizeGenerationTask({
        id: "task-1",
  ...
  it("耗尽任务批量失败并唤醒 callback outbox", async () => {
    const exhausted = { id: "task-1", taskType: "video", status: "failed" };
    databaseMocks.state.executeRows = [{ id: exhausted.id }];
    databaseMocks.state.returningRows = [exhausted];

    await expect(failExhaustedGenerationTasks()).resolves.toEqual([exhausted]);
    const selection = databaseMocks.execute.mock.calls[0]?.[0] as
      | { text?: string }
      | undefined;
    expect(selection?.text).toContain('"attempt_count" >= "max_attempts"');
    expect(selection?.text).toContain("LIMIT 100");
    expect(databaseMocks.state.lastSet).toMatchObject({
      status: "failed",
      resultPayload: null,
      leaseToken: null,
    ...
    expect(databaseMocks.state.lastSet?.callbackStatus).toBeDefined();
  });
});
