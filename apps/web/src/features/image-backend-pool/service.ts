...
import {
  type BackendCooldownSettingKey,
  type BackendFailureRuntime,
  classifyBackendFailure,
  cooldownFromMinutes,
  isClassifiedFailureRecoverable,
  isImageBackendSwitchableError,
  isInvalidBackendCredentialError,
  isMeaningfulSourceCooldownForError,
  isOverloadBackendError,
  isRecoverableBackendError,
  isResetAwareLimitedBackendError,
  isUnsupportedModelBackendError,
  isUsageLimitBackendError,
  parseDateValue,
  resolveAlwaysActiveFailure,
  resolveBackendCooldownMinutes,
  resolveEffectiveFailureForMember,
} from "./backend-error-classification";
import {
...
export {
  ...
  isGroupDisabledBackendError,
  isImageBackendSwitchableError,
  isImageGenDisabledBackendError,
...
const backendFailureRuntime: BackendFailureRuntime = {
  readUnrecoverableKeywords: () =>
    getRuntimeSettingString("IMAGE_BACKEND_UNRECOVERABLE_ERROR_KEYWORDS"),
  readCooldownMinutes: (key, fallback) =>
    getRuntimeSettingNumber(key, fallback, { positive: true }),
};

/** 读取 Sub2API 同步与请求失败分类共用的运行时冷却分钟数。 */
function getBackendCooldownMinutes(key: BackendCooldownSettingKey) {
  return resolveBackendCooldownMinutes(key, backendFailureRuntime);
}

/**
 * 使用运行时系统设置对后端失败分类，同时保持 service 的既有公共接口。
 *
 * @param error 上游或本站包装后的错误文本。
 * @param input 上游显式重置时间与 Retry-After 秒数。
 * @returns 后端状态和冷却截止时间；设置读取失败时显式上抛。
 */
export function classifyFailure(
  ...
  return classifyBackendFailure(error, input, backendFailureRuntime);
}
