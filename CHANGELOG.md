...
- **首页影片 v0.9 穷尽渲染升级**:八项渲染技术升级 + 剧情由七幕 1860vh 扩为八幕 2150vh。技术准入判据:每项技术必须翻译成纸/墨/水/光的物理行为。序幕**活墨**(stable-fluids 第二用途:墨滴落纸真实洇开,浓度由持续渗出与耗散动态平衡,标题从墨晕中显影,prompt 打字时墨被向心流吸向画布——开场那滴墨就是那幅画的原料);显影升级为**生长**(域扭曲分形指状边界 + 湿边暗环 + 由湿到干模糊收敛,进度纯函数可倒放);新增 **macro 凝视幕**(取景窗推近收锋飞白 3.1 倍 + 边缘景深,拉回后画布放大成方形 cover 全屏,dive 从 zoom=1 无缝接管,内容与几何双连续);穿越升级**体积光**(cover 采样修正长宽比拉伸、god rays 光轴、纸纤维暗纹隧道 + 边缘压暗、穿透纸芯的暖光一瞬——白纸段体积光用"影"呈现);增殖粒子升级 **curl-noise 涡流输运**(数值旋度无散流场,墨屑在水流中被携带,墨罩保持到中段衬托粒子对比);**纸面掠光**(fbm 纸纹梯度 x 随滚动缓转的光向,双层颗粒:静纸簇 + 动 IGN);**装裱时刻**(pick 落幅白卡纸 matte 浮现 + 投影加深 + postFlash 玻璃反光一拍 + "你的那张"落款);**墨池地面**(展墙画作倒影 + 地面线,纯 DOM 三档通用)。GPT Image 2 真实素材接收规格见 `docs/plan/2026-07-12-artwork-brief.md`(素材未到不阻塞,程序化资产先行)。
