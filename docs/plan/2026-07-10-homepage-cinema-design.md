...
### GPT Image 2 素材接收规格（已完成）

程序化水墨是当前上限；真实模型生图可再抬高主角质量，且叙事自证
（影片中的画由产品自己生成）。规格与统一化管线见
`docs/plan/2026-07-12-artwork-brief.md`。2026-07-12 全部 16 张经
产品自身 v1 API 生成、审查通过并已替换现役资产。

## 十五、v1.0 剧情线扩展设计（2026-07-12，待用户确认后实施）

用户指令：拉长剧情线，在保留现有的基础上插入更多剧情和介绍，
彰显 GPT2IMAGE PRO 的风采。设计约束不变：产品能力必须叙事化
（每项能力是一个剧情事件，不是功能列表），并通过材质世界观翻译
（纸/墨/水/光/印刷铅字）。

### 主旨升级（四段式，每段一项产品风采）

一句话成一幅画（生成）；一次对话改到满意（编辑）；
一行调用生成万象（API 与批量）；取回属于你的那张（交付）。

### 十幕分镜（2150vh -> 约 2620vh，现有八幕全部保留）

1. opening 260（不变）
2. generate 380（+20，幕内强化：HUD 增加计时器"00:47"随显影跳动
   完成时定格——速度风采；完成态增加分辨率注脚——尺幅风采）
3. macro 210（不变：凝视初稿细节）
4. **revise 220（新）——「再改一笔」（对话式编辑风采）**：
   money shot 之后 prompt 行原句续写"，再画满一点，墨色更沉"
   （同一行接续打字——创作是一场对话）；画布上朱笔细圈沿收笔处
   淡入（传统朱笔批改，手绘抖动，朱砂色第二次合法出场）；
   v2 版本从圈选中心向外域扭曲生长显影，覆盖 v1（denoise 第三
   实例，v1/v2 双纹理交叠）；落幅 HUD "revision 02 · complete"
   静止一拍。资产：需经产品 API 补生成 v1 初稿 enso（墨色略淡、
   笔画略瘦——现役 hero 即 v2 最终稿，dive 及以后全部沿用）。
   因果链：凝视后发现还想改 -> 说一句话 -> 局部重绘 -> 满意 ->
   放大入画。generate 幕显影与 macro 凝视对象改为 v1。
5. dive 200（不变，扎进 v2）
6. manifesto 240（不变）
7. **invoke 190（新）——「一行调用」（API 与批量风采）**：
   墨底延续，等宽字逐字打出（与开场 prompt 打字互文——你会打字，
   你的程序也会）：`POST /v1/images/generations
   {"prompt": "一笔浓墨...", "n": 16}`（不写域名，永真）；回车
   一拍；响应流 16 行 url 逐行滚出（或 16 个墨点逐个点亮），
   细粒子流自行尾涌出；淡出交给 multiply 的画布残影。
   材质翻译：等宽字 = 活字铅字，印刷属于纸墨世界观。
   因果链：宣言（理念）-> 一行调用（手段）-> 增殖（结果）->
   展墙（万象）。darkWindow 恰好涵盖。
8. multiply 260（+10，承接 invoke 的因果）
9. wall 460（不变）
10. pick 200（+30，幕内强化：装裱落幅后画作下方浮出交割单小字
    "PNG · WebP · PSD 分层 · 4K"——交付格式风采，psd-export
    为真实功能）

### 实施要点

- ChapterRail 章窗口：第二章 first=generate last=manifesto 自动
  涵盖 revise；第三章 first 由 multiply 改为 invoke。
- denoise 多实例已支持（keys 参数）；revise 的局部生长 = 现有
  域扭曲显影 + bias 中心改为圈选中心（新 uniform 或复用 crop）。
- invoke 幕以 DOM 为主（打字复用 PromptLine 模式），粒子涌出
  复用 splash 模式改原点。
- i18n 新增：revise 续写句、invoke 注释行、交割单、HUD 文案
  （zh/en 双语）。
- 新资产：v1 初稿 enso 一张（gen-artworks.cjs 加 JOBS 条目，
  subject 加 lighter ink / thinner stroke 限定）。
- 静态回退（static-film）：revise 并入 generate 段展示两版对照，
  invoke 以代码块静态呈现。
