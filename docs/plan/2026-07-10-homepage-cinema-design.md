...
## 十五、v1.0 剧情线扩展设计（2026-07-12，已实施落地）
...
设计修订（2026-07-12 用户反馈）：砍掉计时器等 HUD 注脚式点缀——
特点必须各自成为一段动画剧情，不是小字标注；影片之后的谷段
（SLA/Pricing/FAQ 普通卡片区）同步剧情化，见"谷段三折"。
...
9. wall 460（不变）
10. pick 220（+50，幕内新剧情「分层检视」——交付格式的动画表达：
    装裱落幅后，画作短暂分离为三片错开的层（墨层/纸底层/印章层，
    DOM 3D 微位移 + 层间投影），悬停一拍再合上，随后一行小字
    "PNG · WebP · PSD 分层"点题——特点用动画演，文字只署名。
    psd-export 为真实功能）

### 谷段三折（影片之后的常规流区块剧情化，文档流 + whileInView）

谷段必须保留交互（订阅按钮/FAQ 展开/管理开关），不进 sticky
行程；改造方式 = 视觉语言彻底换成纸墨物件 + 滚动入场剧情。

1. **SLA -> 「千笔之约」墨点阵**：可靠性不是读数字，是亲眼看见。
   入场时最近 1000 张生成化作一片墨点雨，按行列波次落在纸面排成
   点阵（成功 = 墨点，平台错误 = 朱色空圈，审核/用户错误 = 淡灰
   点），随后成功率大数字显影，分类图例以点色对应。白卡片格子
   全部退役。实现：canvas 2D 或分批 DOM 点 stagger，视口触发。
2. **Pricing -> 「润格」立轴**：书画家挂单卖画的价目传统（齐白石
   润格典故）。三档套餐 = 三幅窄长立轴，入场自上方垂落展开
   （scaleY origin-top + 微摆，三张 stagger），上下轴头装帧、
   纸底质感；推荐档系墨底白字签条（不用朱砂——唯一强调色纪律，
   朱砂只属于作品印章与朱笔批改）；价格数字沿用 animated-price
   显影。Card 容器换立轴形制，订阅交互与能力矩阵原样保留。
3. **FAQ -> 「册页」问答**：Accordion 视觉换成册页折子——每问
   一折，页边衬线编号，问句前"问："答句前"答："（古画论问答体）；
   展开时答案文字逐行显影（stagger，墨迹落纸），折痕线分隔;
   整册入场逐条淡入。白纸卡退役。
4. **页边墨线生长（InkThread 升级）**：墨线随滚动 scaleY 生长——
   影片里那滴墨没有停，它沿页边把润格/册页一路缝到终幕，
   在 FinaleStage 再次坠落（尾墨滴已有）。谷段与影片成为同一
   张纸上的连续叙事。
...
- invoke 幕以 DOM 为主（打字复用 PromptLine 模式），粒子涌出
  复用 splash 模式改原点。
- i18n 新增：revise 续写句、invoke 注释行、分层检视署名、
  SLA 千笔标题、FAQ 问答前缀（zh/en 双语）。
...
- 静态回退（static-film）：revise 并入 generate 段展示两版对照，
  invoke 以代码块静态呈现；谷段动画均为 whileInView 增强，
  reduced-motion 下直接呈现终态（内容真相不依赖动画）。

### v1.0.1 谷段跟随化（2026-07-12，已实施落地）

用户反馈：后一小半的卡片要"逐个跟随动画展现"，设计前卫大胆。
定性：v1.0 谷段三折用的是一次性入场动画（whileInView / rAF 时间
轴，播完即止），而影片段的语言是滚动即推轨——本轮把三折全部升级
为**滚动跟随（scrub）**：滚多少演多少、滚回即倒放、快滚有触觉，
影片的镜头语言贯穿到页尾，整页真正成为一部片子。

1. **SLA 千笔之约 scrub 化**：墨点落纸进度从 1.6s rAF 时间轴改为
   滚动进度纯函数（useScroll 元素窗口），滚回收墨；成功率大数字
   随点阵铺满在尾段显影（blur+opacity，mounted 门闩保 SSR 输出
   终态）；IntersectionObserver 退役。
2. **Pricing 润格廊**（主升级）：lg+ 且非减动效时改为 240vh
   sticky 廊道舞台——竖滚驱动轴列横移（与影片展墙"竖滚横推"同
   语言），页边墨线在廊顶横生为挂杆，五幅立轴逐个相位**展卷**：
   clip-path 露纸 + 地杆作为卷筒前沿沿展开线下滚（内容不变形的
   真实卷轴物理），挂绳随挂上绷直；滚动速度经 useVelocity 化作
   全列 ±1.4 度微摆（逐轴相位微差，快滚轴晃停下回稳——全片触觉
   签名）；落幅签条同拍落款、推荐轴描边浮现、静止一拍。横移终点
   取"列尾右对齐"（见勘误 1）。窄屏/减动效/SSR 保留横向 snap
   轮播轨，运行时 matchMedia 切换；轴身 Card 为两轨共用渲染函数
   （pricing-plan-gallery.tsx 纯演出层，业务内容由调用方注入），
   订阅交互与能力清单不动，零新 i18n key。
3. **FAQ 册页翻折跟随**：逐折绑自身视口位置（元素级 useScroll），
   以顶边为轴自 -50 度翻平落下 + spring 纸页重量，先到先翻、
   滚回折回；radix 展开与墨迹扫入不动。

实施勘误（走查实证，与 gl-motion-craft 勘误互补）：

1. **廊道落幅构图须以"按钮可达"为先**：横移终点若取推荐档对中，
   1280 视口下末档全程只露 67px 边缘，订阅按钮永不可达（商业
   缺陷）。终点改为列尾右对齐（最高两档压轴完整亮相，裁掉的只是
   首档 ¥0），宽视口（约 1894+）回退推荐档对中保住镜头量。
2. **sticky 舞台内容必须为紧视口（900px）做预算**：全轴（挂杆/
   绳/卷杆/Card/地杆）超高会把挂杆顶进站点 header 的遮挡带、把
   CTA 压出视口底。收紧共用 Card 密度（gap-5/py-5/清单
   space-y-1.5）+ 舞台 pt-16 justify-start，按钮底 858<900 收纳。
3. **满宽演出段的页边刻度会被展品裁切**：InkThread 标签横排约
   90px，廊道轴身覆盖右页边时露出半截字；移到左页边（横移使左侧
   渐空）。InkThread 新增 labelTop 参数，SLA 段（左栏大数字占
   视口中带）传 78vh 落到下部空白避让。
4. **展卷的裁切层会切掉超出轴顶的签条**：badge 必须渲染在
   clip-path 层之外（共用轴身因此把签条从 Card 内抽出）。
5. clip-path 展开完毕（>=0.995）须撤为 none，否则 hover 投影的
   下缘被裁切线切掉。

走查记录：SLA 落墨中段/完成显影、廊道 p=0.25/0.5/落幅、倒放
卷回、FAQ 逐折翻开与展开交互、390px 轮播回退、落幅三档订阅按钮
elementFromPoint 可点——全部通过。无 JS 下谷段轮播 whileInView
初态问题为存量缺口，登记 TODO。
