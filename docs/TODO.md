...
遗留待办（打磨迭代，非本轮范围）：
- [ ] **GPT Image 2 真实素材接入**：用户生成 16 张（hero enso 2048+ 与 15 幅万象 1024+，prompt 模板见 artwork-brief），实现 ingest 管线（底色归一/统一纸纹/统一朱印/深度图/webp 导出）后逐位替换。
- [ ] **展墙交错格穿地面线**：stripPos 高低交错使部分画作底缘越过地面线，素材更新时统一调几何。
